...
turbine03_init_y = [3537, 3537, 3537, 3537, 3537,
                    3537, 3537, 3537, 3537, 3537]  # 初始输出值
turbine03_init_y = np.array(turbine03_init_y, dtype=np.float32)
turbine03_init_u = [0, 0, 0, 0, 0]  # 初始输入值
...
turbine03_dynamics = NLARXModelWrapper(model,
    ...
    u_init=turbine03_init_u  # 初始值可以根据需要调整
)

turbine03_dynamics.reset() # 调用时记得输入归一化 输出反归一化

# # ====== 3. 加载数据集 ======
# df = pd.read_csv('Turbine03_Data01_final.csv')  # 替换为你的真实数据集路径
...
# # ====== 4. 裁剪数据集 ======
# u_series = u_series[:25749]
# y_series = y_series[:25749]
# # 手动归一化处理
